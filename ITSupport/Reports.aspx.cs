...
public partial class Reports : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            RenderReport("Daily");
            divReportName.InnerHtml = "View Daily Report";
        }
    }
    ...
    private void RenderReport( string type)
    {
        try
        {
            ...
            if (type == "Daily")
            {
                ...
                dtObjReportDetails = BindGridDaily("RPT_DailyRequests");
            }
            else if (type == "Weekly")
            {
                divDailyFilter.Visible = false;
                divWeeklyFilter.Visible = true;
                dtObjReportDetails = BindGridWeekly("RPT_WeekWiseRequest");
            }
    ...
    public DataTable BindGridDaily(string SP)
    {
        try
        {
            ...
            DataTable dt = new DataTable();
            SqlCommand cmd = new SqlCommand(SP, conn);
            cmd.Parameters.Add("@dateFromString", txtFromDate.Text.ToString());
            cmd.Parameters.Add("@dateToString", txtToDate.Text.ToString());
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dt);
            return dt;
